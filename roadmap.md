...
Status: **IN PROGRESS.** The displayed working title is now `Pocket Kingdoms` instead of a protected franchise name. A touch-tested Help & Settings overlay documents core gestures and persists difficulty, guided opener, audio, camera speed, and text/UI scale through `user://preferences.cfg`; audio, camera movement, and window scaling consume those settings at runtime. The four-step opener now includes a compact 68×48 `Skip` control that restores the full HUD and saves the user's guidance choice. Production failures now distinguish population cap, full queues, unfinished or incompatible buildings, and exact resource shortfalls; placement affordability reports the missing amounts and next action. Those player-facing messages are retained in first-session diagnostics, and the successful training path rejects stale failure text. HUD/world target diagnostics refresh from settled layout on wall-clock intervals. Latest regression evidence is phone **PASS (64/64)** and smoke **PASS (18/18)** with clean logs.
...
Status: **IN PROGRESS.** Match conclusions now preserve and display the victory cause, duration, Feudal timing, final ages, army produced (separate from villagers), units/buildings lost, total gathered resources, cumulative sacred-site control, and comparative score. The summary publishes structured diagnostics, guards against duplicate overlays, retains 48 px restart/menu actions, and has a focused 844×390 safe-bounds regression. A real-scene headless suite now passes three production-triggered paths: sacred-site victory → menu, enemy Town Center elimination → menu, and player Town Center loss/defeat → restart into a fresh `PLAYING` match; each also verifies result/reason stability under a duplicate ending event. The seeded balance harness now pins MCP `2.16.1`, accepts seed/difficulty matrices, supports 1×–3× simulation, records attacks and authoritative game-over summaries, and can require completion instead of treating elapsed time as success. After removing premature 12-House expansion, prioritizing the first Barracks, and counting queued villagers toward targets, Medium seed `424242` completed at **10:11** by AI sacred-site victory with first attack **2:54**, peak military **29**, zero runtime errors, and 120 FPS (`docs/seeded_match_probe_latest.json`): **1/10 required seeded completions**. Latest integration evidence remains phone **PASS (64/64)**, smoke **PASS (18/18)**, focused summary **PASS**, and all three ending scenarios **PASS**. One earlier phone run had a non-emitting Scout-button tap (`result=none`) before two later clean runs, so touch reliability remains under observation. Nine more seeded simulations, three manual phone matches, and pacing judgment remain open.
